...
The test server runs the latest stable build with every add-on enabled. You can experience ranked matchmaking, the full shop, generators, team upgrades, and every other mechanic exactly as your players would. **No registration, no whitelist — connect and play immediately.**
...
One payment of **€40.00** grants you a permanent license that covers **all servers you own** — whether you run a single lobby or a 50-server BungeeCord network. There are no monthly fees, no per-server add-ons, and no hidden charges. You receive all future updates for the current major version free of charge.
...
```
%bw_level%           # Player level
%bw_kills%           # Kill count
%bw_deaths%          # Death count
%bw_wins%            # Win count
%bw_beds_broken%     # Beds broken count
%bw_tokens%          # Token balance
%bw_current_arena%   # Current arena name
%bw_team_color%      # Team color
%bw_game_state%      # Current game state
%bw_ranked_elo%      # Ranked ELO rating
%bw_ranked_wins%     # Ranked win count
```
...
- **Price:** **€40.00** — one-time payment, permanent license.
